...
under `packages/desktop/src` or `packages/ui/src` repaints the running app in
80-180 ms through HMR, with no rebuild, no relaunch and no page reload. Only
Bun-side changes (`src/bun`, `@acepe/electrobun-shell`, `@acepe/server`) need
`electrobun:build`.
...
- **Required QA CLI baseline** (from `packages/desktop`): `bun run qa doctor` to prove the app answers, then `bun run qa run` with a heredoc script that inspects the element or region proving the change. `bun run qa help` lists the helpers.
- **Parallel QA.** Each checkout runs its own dev app: `scripts/dev-app.sh` derives a port, launchd label and QA socket from the checkout path, and prints them. Target one with `ELECTROBUN_QA_APP_ID=com.acepe.app.<name> bun run qa ...`; `scripts/dev-app.sh status` lists every running instance. The primary checkout keeps port 1420 and the bare socket. A worktree borrows the primary's app bundle, so it needs `bun install` and not `electrobun:build`. Two agents sharing one app read each other's transcript, which invalidates the evidence.
- **QA evidence must match the bug.** A `snapshotText`/`snapshotDom`/`js` read is enough only for static visual/style changes. Interaction bugs must drive the interaction with `click`, `typeText`, `pasteText`, or `waitForText` inside the same script, then read the resulting DOM. Timing, scroll, streaming, animation, and layout-transition bugs must sample the transition after the code change; a static read is not enough.
- The CLI takes no screenshots on purpose. DOM facts are the evidence.
...
- **Scenario replay.** `?qa=<scenario>` boots the same app shell against a recorded scenario instead of the live server: real stores, real reducer, real streaming cadence, no agent and no tokens. Prefer it for any QA that needs a specific session state (mid-stream, waiting on a permission) over driving a real agent. `bun run qa capture --session <id> --name <n>` turns a live session into one. Scenarios live in `packages/qa-scenario`.
- `bun run qa` wraps the `electrobun-qa` CLI in `packages/electrobun-qa`. It reaches the WebView over a unix socket at `/tmp/electrobun-qa/<app-id>.sock`, which unsigned builds open through a preload. There is no debugging port and no Tauri bridge.
- A successful `bun run qa run` writes `.codex/state/ui-qa-evidence.json`, the stamp the Codex Stop hook checks.
...
- For desktop app QA, use `bun run qa`. It attaches to the running Electrobun WebView so you can read the DOM and app state, click, and type inside the desktop context. A normal browser at `localhost:1420` is not enough for Acepe desktop QA because Electrobun APIs are missing there.

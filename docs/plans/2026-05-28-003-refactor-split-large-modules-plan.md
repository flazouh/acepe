...
- ✅ **U8** `agent-panel.svelte` → **deepened** (committed `bcbf9f01a`, `53d6a7c84`). Beyond the initial pure-helpers (buildTokenRevealCss, hasStreamingPreviewContent), extracted `agent-panel-interaction-handlers.ts` (question/tool-file/plan-permission factory), `agent-panel-export-handlers.ts` (copy/finder/raw/export factory), and moved `buildTodoMarkdown` into pure-helpers. 2,803 → 2,638 lines; `bun run check` exit 0; biome clean. **Principled stop:** remaining bulk is irreducible controller logic (`$derived` blocks + `$state`-writing handlers); further extraction would need setter/getter indirection (anti-pattern flagged by thermo-nuclear review).

...
### 8.1 Execution correction — the live graph must be held, not reconstructed

The July 13 implementation audit found that Phase 1 is not complete even though live
transcript updates call `fold_step`. `transcript_projection/runtime.rs` currently rebuilds a
temporary `SessionStateGraph` from only the current `TranscriptSnapshot`, folds one event,
then copies transcript entries back. In parallel, `ProjectionRegistry`,
`SessionSupervisor`/`SessionGraphRuntimeRegistry`, and the transcript registry still reduce
the same `SessionUpdate` independently. This is a dual-write architecture and blocks Phase
4 deletion.

The corrected dependency order is:

```text
held live graph → async history source → provider physical moves/naming
                → graph-derived delivery → compatibility deletion
```

#### Slice A — one held live `SessionStateGraph` (critical path)

Authority seam: `SessionGraphRuntimeRegistry` owns the full graph for every attached live
session, not only lifecycle checkpoints and transcript-row ledgers.

- Add a keyed full-graph holder inside `SessionGraphRuntimeRegistry` with explicit
  `seed_graph`, `graph_for_session`, `apply_provider_event`, and `remove_session` behavior.
- Seed new sessions from `FoldContext`; seed restored sessions with the graph returned by
  `fold_full`. Reconnect attaches transport to that held graph; it does not rebuild truth.
- Normalize one live raw update to one ordered `ProviderEvent` at the dispatcher boundary.
  `persist_dispatch_event` then applies that event once through `fold_step` and receives the
  previous graph, next graph, and graph delta needed for delivery.
- Derive transcript, operation, interaction, lifecycle, activity, turn state, failure, and
  revision envelopes from the held graph transition.
- In the same cutover, remove the parallel truth writes from `ProjectionRegistry`,
  `TranscriptProjectionRegistry`, and lifecycle-only `SessionGraphRuntimeSnapshot` for the
  migrated event families. Do not leave fallback reads or parallel writes.
- Keep journal allocation/persistence as ordering and durability infrastructure. The journal
  supplies `event_seq`; it does not build a second projection.

TDD seams already approved by this plan:

1. `SessionGraphRuntimeRegistry::apply_provider_event`: sequential user, assistant, tool,
   interaction, and terminal-turn events remain together in one held graph.
2. `persist_dispatch_event`: one journaled update causes exactly one fold transition and the
   emitted envelope equals a projection of the held graph.
3. Reconnect/open: a graph seeded from `fold_full(history_events)` accepts the next live event
   without losing operations, transcript entries, interactions, or persisted revision.
4. `fold_full(events)` equals sequential `fold_step` over the same persisted event region.

Deletion gate for Slice A: production live ingress must not call
`graph_from_transcript_snapshot`, `ProjectionRegistry::apply_session_update*`, or
`TranscriptProjectionRegistry::apply_session_update*` as truth builders.

#### Slice B — make `HistorySource` asynchronous

`HistorySource::read` currently has a synchronous signature while several provider
implementations invoke async disk loaders with `Handle::block_on`. That can panic when replay
runs inside a Tokio runtime.

- Change the source contract to an async-returning future (matching the existing provider
  traits) and await it from history delivery/indexing.
- Remove every provider-side `Handle::block_on`/nested-runtime bridge.
- Preserve ordered `Vec<ProviderEvent>` output and typed `HistoryError` semantics.
- Add an async conformance test that reads every registered fixture from an active Tokio
  runtime; nested-runtime execution must not panic.

#### Slice C — provider and naming consolidation

Only after Slices A and B are green:

- Finish physical provider moves and direct `ProviderEvent` history emission.
- Move `reconciler` physically to `ingress/tool_identity` and rename `*Adapter` tables to
  `ToolTable` without keeping aliases.
- Replace `AgentParser` with provider-owned `LiveSource` implementations after all raw parsing
  entry points route through the plugin registry.
- Do **not** mechanically rename `AgentProvider` to `Transport`: first split spawn/connect/IO
  from authentication, capability, model/mode, presentation, and history policies. Reuse or
  rename the existing `acp::transport::TransportAdapter`; do not introduce a second transport
  seam.
- Define plugin behavior for `Custom` and `Forge` explicitly. A provider with no history uses
  a typed unsupported/optional history capability, not a fake empty history source.

#### Slice D — graph-derived delivery and final deletion

After the live holder is the sole authority:

- Move open result, snapshot, delta, viewport, and scene carving under `session/delivery`.
- Delete `CanonicalTranscriptEvent`, `SessionThreadSnapshot`,
  `ProviderOwnedSessionSnapshot`, `MaterializedThreadSnapshot`,
  `TranscriptSnapshot::from_stored_entries`, and relink/ensure/drop repairs.
- Keep `StoredEntry` only as a private provider parse scratch type and a one-way export DTO;
  it must never be accepted by the engine or delivery as truth input.
- Replace the current compatibility allowlist in `session/tests/deletion_gate.rs` with absence
  assertions. A confinement test is an intermediate guard, not migration completion.

...
| File | Holds | Rule |
|------|-------|------|
| `Provider.ts` | id, capabilities, presence probe, executable resolution | no session state, no translation |
| `Facts.ts` | the contract fact vocabulary, fact constructors, terminal-fact predicate | schemas and literals only |
| `Map.ts` | raw provider message becomes contract facts, plus the stream state it needs | pure. no Effect, no I/O, no clock |
...
- Split `Adapter.ts` further only when a concern is genuinely separable and the file is large. Use `Permissions.ts`, `Watchdog.ts`, `Process.ts`, `Session.ts`, in that vocabulary, and keep `Adapter.ts` as the spine that names and orders them.
- Keep the shared kernel flat in `Layers/`: `ProviderRegistry.ts`, `ProviderAdapterRegistry.ts`, `ProviderBridge.ts`, `AgentInstaller.ts`. These know every provider and no provider's details.

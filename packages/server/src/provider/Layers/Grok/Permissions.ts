...
export type GrokRespondToPermissionInput = {
	readonly sessionId: SessionId
	readonly permissionId: string
	readonly decision: GrokPermissionDecision
}
...
const optionKindAllows = (kind: string, decision: GrokPermissionDecision): boolean => {
	if (decision === "allow") {
...
const freeApprovalId = (
	pending: HashMap.HashMap<string, Deferred.Deferred<GrokPermissionDecision>>,
	base: string,
	attempt: number
): string => {
...
export const decidePermission = Effect.fn("GrokAdapter.decidePermission")(function*(
	...
	const deferred = yield* Deferred.make<GrokPermissionDecision>()
...
// re-confirmed.
const ABANDONED_DECISION: GrokPermissionDecision = "deny"
...
// the turn never ends, the spawned `grok` subprocess stays alive,
...
export const drainPendingPermissions = Effect.fn("GrokAdapter.drainPendingPermissions")(
...
export const respondToPermission = Effect.fn("GrokAdapter.respondToPermission")(function*(

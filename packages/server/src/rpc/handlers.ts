...
import {
	...
	CheckpointService,
} from "../checkpoint/Services/CheckpointService.ts";
import {
	FileIndexNotADirectoryError,
	FileIndexRootNotFoundError,
} from "../fileIndex/Errors.ts";
import {
	type FileIndexError,
	FileIndexService,
} from "../fileIndex/Services/FileIndexService.ts";
...
} from "../git/Services/GitService.ts";
import {
...
import type {
	HistoryImportError,
	HistoryImporterShape,
} from "../history/importer.ts";
...
import {
	forgetResolvedProjectIcon,
	resolveProjectIcon,
} from "../projects/projectIconResolver.ts";
...
export const toRpcProject = (
	project: SessionProjectionSnapshot["projects"][number],
) => ({
...
export const toRpcCheckpoint = (
	checkpoint: SessionProjectionSnapshot["checkpoints"][number],
) => ({
...
const decodeRpcFileGitStatuses = Schema.decodeUnknownEffect(
	Schema.Array(FileGitStatus),
);
...
const liveProjectGitStatus = Effect.fn("liveProjectGitStatus")(function* (
	...
	return yield* git.projectGitStatus(workspaceRoot).pipe(
		...
		Effect.tapCause((cause) =>
			Effect.logWarning(
				`project git status unavailable: ${Cause.pretty(cause)}`,
			),
		),
...
const withCheckpointFiles = Effect.fn("withCheckpointFiles")(function* (
	...
	const rows = yield* Effect.forEach(snapshot.checkpoints, (row) =>
		checkpoints.getFileSnapshots(row.sessionId, row.checkpointId).pipe(
			...
			Effect.tapCause((cause) =>
				Effect.logWarning(
					`checkpoint files unavailable: ${Cause.pretty(cause)}`,
				),
			),
...
export const rpcSnapshotForRequest = Effect.fn("rpcSnapshotForRequest")(
...
export type ProviderDiscoveryRpcErrorInput =
	| PlatformError
	| SqlError
	| Schema.SchemaError;
...
export const toHistoryImportRpcError = (
	error: HistoryImportError,
): RpcServerError => {
	if (
		error._tag === "PlatformError" ||
		error._tag === "HistoryDirectoryNotFoundError"
	) {
		return new RpcSchemaError({ issue: error.message });
	}
	return toRpcError(error);
...
export const resolveHistoryImportProjectId = Effect.fn(
	"resolveHistoryImportProjectId",
)(function* (
	...
	const found = Arr.findFirst(
		existing,
		(project) =>
			project.workspaceRoot === workspaceRoot && project.deletedAt === null,
	);
...
export const importProviderSessionHandler = Effect.fn(
	...
	const projectId = yield* resolveHistoryImportProjectId(
		projects,
		request.projectPath,
	);
	const result = yield* claudeHistory.importSessionFile(
		{
			...
			projectId,
			workspaceRoot: request.projectPath,
		},
...
const readAllFrom = Effect.fn("readAllFrom")(function* (
	...
	while (true) {
		const page = yield* Stream.runCollect(
			store.readFrom(cursor, EVENT_PAGE_SIZE),
		);
...
export const eventsFromSequence = (
	...
	Stream.unwrap(
		Effect.gen(function* () {
			const liveQueue = yield* Queue.unbounded<
				OrchestrationEvent,
				Cause.Done
			>();
			yield* engine.streamDomainEvents.pipe(
				Stream.runForEach((event) =>
					Queue.offer(liveQueue, event).pipe(Effect.asVoid),
				),
				Effect.ensuring(Queue.end(liveQueue).pipe(Effect.asVoid)),
				Effect.forkScoped({ startImmediately: true }),
			);
			const replayed = yield* readAllFrom(store, fromSequence).pipe(
			...
			return Stream.concat(
				...
				Stream.fromQueue(liveQueue).pipe(
					Stream.filter((event) => event.sequence > last),
				),
			);
		}),
	);

export const dispatchOrchestrationCommand = Effect.fn(
 ...
 */
export const listProviderSessionsHandler = Effect.fn(
	"listProviderSessionsHandler",
)(function* (
	providerDiscovery: ProviderSessionDiscoveryShape,
	projects: ProjectionProjectsShape,
	request: ListProviderSessionsRequest,
) {
	const discovered = yield* providerDiscovery.listSessionsForProject(
		request.projectPath,
	);
	...
	const match = Arr.findFirst(
		...
		(project) =>
			...
			project.workspaceRoot === request.projectPath,
	);
...
export const RpcHandlersLive = AcepeRpc.toLayer(
	Effect.gen(function* () {
		...
		return {
			...
			events: (request) =>
				eventsFromSequence(store, engine, request.fromSequence),
			getProjectIndex: (request) =>
				fileIndex
					.getProjectIndex(request.projectPath)
					.pipe(Effect.mapError(toFileIndexRpcError)),
			invalidateProjectIndex: (request) =>
				// Also drop the resolved icon. Detection is memoised per project,
				// so without this a logo added to a project after the fact stays
				// invisible until the app restarts. Refreshing a project's files
				// is exactly when its icon is worth looking for again.
				Effect.sync(() => forgetResolvedProjectIcon(request.projectPath)).pipe(
			...
			importProviderSession: (request) =>
				importProviderSessionHandler(
					...
					projects,
					request,
				).pipe(Effect.mapError(toHistoryImportRpcError)),
		};
	}),
);

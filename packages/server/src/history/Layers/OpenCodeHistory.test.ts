...
import { OpenCodeHistoryLive } from "./OpenCodeHistory.ts"
...
const TestLive = OpenCodeHistoryLive.pipe(
	...
	messages: [
		{
			sessionId: "opencode-session",
			sequence: 3,
			messageId: "opencode-session:user:0",
			turnId: null,
			rowType: "user",
		...
		},
		{
			sessionId: "opencode-session",
			sequence: 4,
			messageId: "opencode-session:assistant:0",
			turnId: null,
			rowType: "assistant",
...
Vitest.layer(isolated())("OpenCodeHistoryLive", (it) => {
	it.effect("imports JSONL into projections and skips malformed lines", () =>
		Effect.gen(function*() {
			...
			const history = yield* OpenCodeHistory
			const dir = yield* fs.makeTempDirectoryScoped()
			yield* fs.writeFileString(
				path.join(dir, "opencode-session.jsonl"),
				[
					'{"id":"msg-user-1","role":"user","parts":[{"type":"text","text":"Hello OpenCode"}]}',
					"not-json",
					'{"id":"msg-assistant-1","role":"assistant","parts":[{"type":"text","text":"Hi there"}]}',
					""
				].join("\n")
			)

...
const projectTurnCompleted = (
	...
	decodePayload(TurnCompletedPayload, event.payload, event.type, "payload").pipe(
		Effect.map((payload) =>
			updateSession(model, payload.sessionId, (session) => ({
				...session,
				updatedAt: event.occurredAt
...
export const projectEvent = (
	...
	return Match.type<OrchestrationEvent>().pipe(
		Match.discriminatorsExhaustive("type")({
			...
			VoiceAmplitudeObserved: () => Effect.succeed(model),
			VoiceModelDownloadProgressed: () => Effect.succeed(model),

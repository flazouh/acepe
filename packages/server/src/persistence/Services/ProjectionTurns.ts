...
// before Acepe read the file, and nothing closes it from outside --
// ProviderBridge deliberately never answers an imported prompt. So an
// imported turn opens and ends in one step.
const finishedIfImported = (
	turn: ProjectedTurn,
	origin: TranscriptFactOrigin | undefined,
	at: IsoDateTime
): ProjectedTurn => (origin === "imported" ? completeTurn(turn, at) : turn)
			...
			return decodeTurnId(payload.messageId).pipe(
				Effect.map((turnId) => {
					...
					return Arr.append(
						...
						finishedIfImported(started, payload.origin, event.occurredAt)
					)
				})
			)
		})
...
const projectTokenAppended = (
	...
	decodePayload(TokenAppendedPayload, event.payload).pipe(
		Effect.flatMap((payload) => {
			...
			// An imported turn is closed the moment it starts, so the
			// imported assistant text that follows finds no open turn. It
			// still belongs to the turn right above it: its own message id
			// is the provider's assistant id, not a turn id anything else
			// refers to, so starting a turn from it would double every
			// imported turn and leave the new one running.
			const lastTurn = Arr.last(current)
			if (payload.origin === "imported" && Option.isSome(lastTurn)) {
				return Effect.succeed(replaceTurn(current, addOutputToken(lastTurn.value)))
			}
			return decodeTurnId(payload.messageId).pipe(
				Effect.map((turnId) => {
					const started = startTurn({

...
import { BellSimple } from "../icons/index.js";
...
{#if totalCount > 0}
	{#if collapsed}
		<button
			type="button"
			class="flex h-8 w-full items-center justify-center rounded-lg border border-border/50 bg-card/75 text-muted-foreground transition-colors duration-200 hover:bg-accent/50 hover:text-foreground"
	...
	{:else}
		<div class="mb-0.5 flex shrink-0 flex-col overflow-hidden rounded-lg border border-border/50 bg-card/75 transition-[transform,opacity] duration-200 ease-out">
			<button
				type="button"
			...
			{#if expandedInternal}
				<div class="flex flex-col gap-0.5 p-0.5 pt-0">
					{#each groups as group (group.id)}
						{#if group.items.length > 0 || emptyHint}
							<div class="flex min-w-0 flex-1 flex-col overflow-hidden rounded-lg bg-card/75">
								<FeedSectionHeader
									sectionId={group.id}
									label={group.label}
								...
								{#if group.items.length > 0}
									<div class="flex flex-col gap-0.5 p-0.5">
										{#each group.items as item}
											<div
												class="overflow-hidden rounded-lg border border-border/60 bg-accent/30 transition-all duration-150 hover:border-border hover:bg-accent/45"
											>

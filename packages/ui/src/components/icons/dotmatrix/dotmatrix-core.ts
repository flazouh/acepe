/** Inline style map for dot-matrix cells (CSS custom properties + layout). */
export type DmxStyleProperties = Record<string, string | number | undefined>;
...
export function createPathWaveResolver(getPathNorm: NormFn): DotAnimationResolver {
  return ({ isActive, row, col, index, reducedMotion, phase }) => {
    ...
    const style = { "--dmx-path": path } as DmxStyleProperties;
...
export interface DotMatrixBaseRenderInput {
  size: number;
  dotSize: number;
  color: string;
  colorPreset?: DotMatrixColorPreset;
...
export interface DotCellRenderOutput {
  className: string;
  style: DmxStyleProperties;
}

export function renderDotCell(
  input: {
    index: number;
    row: number;
    col: number;
  ...
  const style: DmxStyleProperties = {
    width: dotSize,
    height: dotSize,
  ...
  if (animationState.style) {
    const animStyle = animationState.style;
    for (const key of Object.keys(animStyle)) {
      const value = animStyle[key];
      if (value !== undefined) {
        style[key] = value;
      }
    }
  ...
  if (isActive) {
    ...
    if (typeof rawOpacity === "number") {
      style.opacity = remapOpacityToTriplet(rawOpacity, ob, om, op);
      const parts = dmxDotBloomParts(true, rawOpacity, bloom, halo, ob, om, op);
      style["--dmx-bloom-level"] = parts.level;
...
export function buildDotMatrixRootStyle(input: {
  size: number;
  dotSize: number;
  color: string;
  colorPreset?: DotMatrixColorPreset;
  ...
  const safeSpeed = input.speed > 0 ? input.speed : 1;
  const speedScale = 1 / safeSpeed;
  const { gap, matrixSpan } = getMatrix5Layout(input.size, input.dotSize, input.cellPadding);
  ...
  const ob = clamp01Dmx(input.opacityBase);
  const om = clamp01Dmx(input.opacityMid);
  const op = clamp01Dmx(input.opacityPeak);
  const { resolvedColor, dotFill } = resolveDmxColorTokens(input.color, input.colorPreset);

  const rootStyle: DmxStyleProperties = {
    width: matrixSpan,
    height: matrixSpan,
    "--dmx-speed": speedScale,
    "--dmx-dot-size": `${input.dotSize}px`,
    "--dmx-halo-level": input.halo,
  ...
  } else if (input.minSize !== undefined) {
    rootStyle.minWidth = input.minSize;
    rootStyle.minHeight = input.minSize;
  }
...
export function dotMatrixRootClassNames(input: {
  dotShape: DotShape;
  muted: boolean;
  bloom: boolean;
  halo: number;
  ...
  return cx(
    "dmx-root",
    `dmx-dot-shape-${input.dotShape}`,
    input.muted && "dmx-muted",
    dmxBloomRootActive(input.bloom, input.halo) && "dmx-bloom",
    dmxBloomHaloSpreadClass(input.halo),
    !input.useWrapper && input.className,
  );
}

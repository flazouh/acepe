	...
	{#if hasScript}
		<div>
			{#if isScriptCollapsible}
				...
				>
					<RoundedIcon name="chevron-right" class="size-3 shrink-0 transition-transform duration-150 {isScriptExpanded ? 'rotate-90' : ''}"
					/>
...
{scriptPreview}</pre
	...
	{#if hasDetails && detailsText}
		<div>
			...
			>
				<RoundedIcon name="chevron-right" class="size-3 shrink-0 transition-transform duration-150 {isResultExpanded ? 'rotate-90' : ''}"
				/>

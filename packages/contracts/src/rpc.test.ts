...
const unusedDispatch: RpcTransport["dispatch"] = (_command) =>
	Effect.succeed({ sequence: 0 });
const unusedSnapshot: RpcTransport["snapshot"] = (_request) =>
...
describe("Schema-encoded boundary", () => {
	...
	it("round-trips orchestration commands as dispatch payload", () => {
		const encoded = Effect.runSync(
			Schema.encodeUnknownEffect(OrchestrationCommand)(createProject),
		);
		const decoded = Effect.runSync(
			Schema.decodeUnknownEffect(OrchestrationCommand)(encoded),
		);
	...
	it("round-trips a default shell result through Exit", () => {
		const encoded = Exit.succeed("/bin/zsh").pipe(
			encodeGetDefaultShellExit,
			Effect.runSync,
		);
	...
	it("round-trips a session snapshot", () => {
		const encoded = Effect.runSync(
			Schema.encodeUnknownEffect(RpcSessionSnapshot)(snapshot),
		);
		const decoded = Effect.runSync(
			Schema.decodeUnknownEffect(RpcSessionSnapshot)(encoded),
		);
	...
	it("round-trips a tool activity's output on a session snapshot", () => {
		...
		const encoded = Effect.runSync(
			Schema.encodeUnknownEffect(RpcSessionSnapshot)(withOutput),
		);
		const decoded = Effect.runSync(
			Schema.decodeUnknownEffect(RpcSessionSnapshot)(encoded),
		);
	...
	it("round-trips git status on a projected project", () => {
		...
		const encoded = Effect.runSync(
			Schema.encodeUnknownEffect(RpcSessionSnapshot)(withGit),
		);
		const decoded = Effect.runSync(
			Schema.decodeUnknownEffect(RpcSessionSnapshot)(encoded),
		);
	...
	it("round-trips projected checkpoints on a session snapshot", () => {
		...
		const encoded = Effect.runSync(
			Schema.encodeUnknownEffect(RpcSessionSnapshot)(withCheckpoint),
		);
		const decoded = Effect.runSync(
			Schema.decodeUnknownEffect(RpcSessionSnapshot)(encoded),
		);
	...
	it("round-trips a library snapshot Exit across JSON IPC", () => {
		const encoded = Exit.succeed(snapshot).pipe(
			encodeSnapshotExit,
			Effect.runSync,
		);
		const decoded = decodeSnapshotExit(
	...
	it("decodes library, settings, skills, voice, git, project, session, and legacy snapshot requests", () => {
		...
		expect(Effect.runSync(decodeSnapshotRequest({ sessionId }))).toEqual({
			sessionId,
		});
	});

	it("maps snapshot requests onto library, settings, skills, voice, git, mcp, project, or session scope", () => {
		expect(snapshotScope(librarySnapshotRequest())).toEqual({
			kind: "library",
		});
		expect(snapshotScope(settingsSnapshotRequest())).toEqual({
			kind: "settings",
		});
		...
		expect(snapshotScope({ sessionId })).toEqual({
			kind: "session",
			sessionId,
		});
	});
				...
				Effect.map((attempt) => {
					...
					if (attempt === failOn) {
						return Stream.fromArray(Arr.take(remaining, 2)).pipe(
							Stream.concat(
								Stream.fail(
									new RpcTransportError({ reason: "bun process killed" }),
								),
							),
						);
					}
		...
		Effect.runPromise(
			Effect.gen(function* () {
				...
				const events = yield* Stream.runCollect(
					client.events(0).pipe(Stream.take(5)),
				);
...
describe("RpcServerError message getters", () => {
	...
	it("RpcFileIndexRootNotFoundError.message carries the path", () => {
		const error = new RpcFileIndexRootNotFoundError({
	...
	it("RpcFileIndexNotADirectoryError.message carries the path", () => {
		const error = new RpcFileIndexNotADirectoryError({
			path: "/tmp/acepe/file.txt",
		});
		expect(error.message.length).toBeGreaterThan(0);
	...
	it("RpcFsPathDeniedError.message carries the offending path", () => {
		const error = new RpcFsPathDeniedError({

<script lang="ts">
	 ...
	 */
	import { GitPanelLayout, type GitStatusFile, type GitLogEntry, type GitStashEntry, type GitRemoteStatus } from '@acepe/ui';

	const ICON_BASE_PATH = '/svgs/icons';

	const allFiles: GitStatusFile[] = [
		{ path: 'src/lib/auth/jwt.ts', indexStatus: null, worktreeStatus: 'modified', additions: 24, deletions: 6 },
		{ path: 'src/lib/auth/session.ts', indexStatus: null, worktreeStatus: 'modified', additions: 8, deletions: 18 },
		{ path: 'src/middleware/auth.ts', indexStatus: null, worktreeStatus: 'modified', additions: 12, deletions: 4 },
		{ path: 'tests/auth.test.ts', indexStatus: null, worktreeStatus: 'untracked', additions: 47, deletions: 0 }
	];
	...
	let logEntries = $state<GitLogEntry[]>([
		...
		{ sha: 'f0e1d2c3b4a5f0e1', shortSha: 'f0e1d2c', message: 'chore: update jose dependency', author: 'CI Bot', date: '2d ago' }
	]);
	...
	function handleStage(path: string) {
		...
		stagedFiles = [
			...
			{
				...file,
				indexStatus: file.worktreeStatus === 'untracked' ? 'added' : (file.worktreeStatus as 'modified' | 'deleted'),
				worktreeStatus: null
			}
		];
	}
	...
	function handleUnstage(path: string) {
		...
		unstagedFiles = [
			...
			{
				...file,
				worktreeStatus: file.indexStatus === 'added' ? 'untracked' : (file.indexStatus as 'modified' | 'deleted'),
				indexStatus: null
			}
		];
	}
	...
	function handleStageAll() {
		stagedFiles = [
			...
			...unstagedFiles.map((f) => ({
				...f,
				indexStatus: (f.worktreeStatus === 'untracked' ? 'added' : f.worktreeStatus) as GitStatusFile['indexStatus'],
				worktreeStatus: null
			}))
		];
		unstagedFiles = [];

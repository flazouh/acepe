<script lang="ts">
	...
	function handleStage(path: string) {
		...
		stagedFiles = [
			...stagedFiles,
			{ ...file, indexStatus: file.worktreeStatus === 'untracked' ? 'added' : (file.worktreeStatus as 'modified' | 'deleted'), worktreeStatus: null }
		];
	}
	...
	function handleUnstage(path: string) {
		...
		unstagedFiles = [
			...unstagedFiles,
			{ ...file, worktreeStatus: file.indexStatus === 'added' ? 'untracked' : (file.indexStatus as 'modified' | 'deleted'), indexStatus: null }
		];
	}

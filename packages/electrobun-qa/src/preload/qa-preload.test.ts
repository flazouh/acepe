...
describe("qa-preload", () => {
	...
	it.effect("click by text mutates the page so a later snapshot sees the change", () =>
		Effect.sync(() => {
			const page = createTogglePage()
	...
	it.effect("handleQaMethod covers every registered qa handler", () =>
		Effect.sync(() => {
			const page = createTogglePage()
			expect(handleQaMethod(page, "qa:snapshotText", {})).toBe("Acepe\n  Toggle\n  Closed")
			...
			expect(handleQaMethod(page, "qa:type", { text: "hello", selector: "#toggle" })).toBe(true)
			expect(handleQaMethod(page, "qa:key", { key: "Enter" })).toBe(true)
			expect(handleQaMethod(page, "qa:scroll", { x: 0, y: 40 })).toBe(true)
			expect(handleQaMethod(page, "qa:eval", { source: "document.title" })).toBe("Acepe")
			expect(handleQaMethod(page, "qa:unknown", {})).toBeNull()
		}),
	)

	it.effect("injectable script registers the qa namespace on the internal bridge", () =>
		Effect.sync(() => {
			...
			expect(qaPreloadScript.includes("__electrobunInternalBridge")).toBe(true)
			expect(qaPreloadScript.includes("receiveInternalMessageFromBun")).toBe(true)
	...
	it.effect("preload script answers qa requests over the internal bridge", () =>
		Effect.gen(function* () {
			...
			const window = {
				...
				__electrobunQa: undefined as { readonly dispatch: (request: unknown) => void } | undefined,
				__electrobun: {
			...
			const run = new Function("window", "document", `${qaPreloadScript}\nreturn window.__electrobunQa;`)
			...
			const batch = yield* Schema.decodeUnknownEffect(QaInternalBatchJson)(first)
			const packetLine = batch[0]
			expect(packetLine !== undefined).toBe(true)
			if (packetLine === undefined) {
				return
			}
			const packet = yield* Schema.decodeUnknownEffect(QaInternalMessageJson)(packetLine)

...
} from "../../../store/permission-operation-projection.js";
...
describe("permission visibility", () => {
	it("treats a permission with a matching tool-call entry as represented", () => {
		...
		expect(isPermissionRepresentedByOperation(permission, "session-1", operationStore)).toBe(true);
	});
	...
	it("keeps orphan permissions visible when no matching tool-call entry exists", () => {
		...
		expect(isPermissionRepresentedByOperation(permission, "session-1", operationStore)).toBe(false);
		expect(visiblePermissionsForOperations([permission], operationStore)).toEqual([permission]);
	});
	...
	it("keeps anchored permissions visible in the session-level permission bar", () => {
		...
		expect(
			visiblePermissionsForOperations([anchoredPermission, orphanPermission], operationStore)

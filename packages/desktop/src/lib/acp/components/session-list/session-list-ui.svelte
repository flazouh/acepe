...
<div class="flex flex-col h-full gap-2" data-thread-list-scrollable>
		...
		<div class="flex flex-col flex-1 min-h-0 gap-0.5">
			{#each sessionGroups as group (group.projectPath)}
				...
				>
					...
					>
						...
						{:else}
							...
							<ProjectHeader
								...
								projectName={group.projectName}
								expanded={isExpanded}
								class="flex-1 min-w-0 group cursor-pointer hover:bg-accent/50 transition-colors"
							>
								{#snippet actions()}
									<div
										class="flex items-center"
					...
					<!-- Git footer with branch picker (hidden when project section is collapsed) -->
					{#if isExpanded}

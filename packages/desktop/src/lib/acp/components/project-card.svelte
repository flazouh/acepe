<script lang="ts">
import { DiffPill, ProjectLetterBadge, HugeiconsIcon } from "@acepe/ui";
			...
			<div class="flex items-center h-6 px-2.5 gap-2 border-t border-border/30">
				{#if data.branch}
					<div class="flex items-center gap-0.5 min-w-0">
						<HugeiconsIcon name="branch" class="size-2.5 shrink-0 text-muted-foreground/40" />
				...
				{#if data.ahead && data.ahead > 0}
					<div class="flex items-center gap-0.5 shrink-0" title="{data.ahead} ahead of remote">
						<HugeiconsIcon name="arrow-up" class="size-2 text-success" />
				...
				{#if data.behind && data.behind > 0}
					<div class="flex items-center gap-0.5 shrink-0" title="{data.behind} behind remote">
						<HugeiconsIcon name="arrow-up" class="size-2 rotate-180 text-warning" />

 ...
 * });
 * Result.match(result, {
 *   onSuccess: (threads) => console.log("Filtered threads:", threads),
...
export function aggregateThreads(
	...
	filter?: ThreadFilter
): Result.Result<SessionDisplayItem[], Error> {
	...
	return Result.succeed(filtered);
}
 ...
 */
export function validateFilter(filter: ThreadFilter): Result.Result<void, Error> {
	// Validate date range if provided
	if (filter.dateRange) {
		const { start, end } = filter.dateRange;
		if (start.getTime() > end.getTime()) {
			return Result.fail(new Error("Date range start must be before or equal to end"));
		}
	}

	// Validate arrays are not empty (empty arrays mean "no matches")
	if (filter.projectPaths && filter.projectPaths.length === 0) {
		return Result.fail(new Error("projectPaths array cannot be empty"));
	}
	if (filter.agentIds && filter.agentIds.length === 0) {
		return Result.fail(new Error("agentIds array cannot be empty"));
	}
	if (filter.sessionIds && filter.sessionIds.length === 0) {
		return Result.fail(new Error("sessionIds array cannot be empty"));
	}

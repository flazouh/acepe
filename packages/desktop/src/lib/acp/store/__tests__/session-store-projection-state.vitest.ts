...
describe("SessionStore.applySessionStateEnvelope", () => {
	...
	it("sends the first prompt for a created reserved session without forcing resume first", async () => {
		...
		const result = await Effect.runPromise(
			Effect.result(
				store.connection.sendMessage("session-1", "cursor UI diagnostic ping - reply ok")
			)
		);
	...
	// The next four cover clearing the optimistic pending send once the canonical
	// transcript acknowledges it. The acknowledgement match lives in the reducer,
	// on canonical-authority surface: envelope-reducer/pending-send-acknowledgement.ts
	// matches a canonical user entry against the pending attemptId, and both the
	// snapshot path (reduceReplaceGraph) and the delta path (reduceTranscriptDelta)
	// emit a clearAcknowledgedPendingSendIntent patch from it.
	//
	// Before that match existed, pendingSendIntent was cleared in only two places,
	// both in services/session-messaging-service.ts: the turn-terminal side effect,
	// and a 90s timeout. It bit when lastTerminalTurnId did not change between the
	// send and the response, which the "completed canonical snapshot" case below
	// reproduces: the optimistic message stayed on screen, and
	// hasLocalPendingSendIntent kept driving connection phase and tab urgency,
	// until the timeout fired. The timeout is now a backstop for a send that is
	...
	it("clears pending send intent only when canonical user attemptId matches", async () => {
		...
		const result = await Effect.runPromise(
			Effect.result(store.connection.sendMessage("session-1", "cursor canonical handoff test"))
		);
		expect(Result.isSuccess(result)).toBe(true);
	...
	it("clears pending send intent when a canonical transcript delta accepts the send", async () => {
		...
		const result = await Effect.runPromise(
			Effect.result(store.connection.sendMessage("session-1", "cursor delta acceptance test"))
		);
		expect(Result.isSuccess(result)).toBe(true);
	...
	it("clears pending send intent when the canonical turn completes without a user attempt id", async () => {
		...
		const result = await Effect.runPromise(
			Effect.result(store.connection.sendMessage("session-1", "codex terminal cleanup test"))
		);
		expect(Result.isSuccess(result)).toBe(true);
	...
	// lastTerminalTurnId does not change here, so the turn-terminal clear never
	// fires: only the canonical transcript acknowledgement retires this send.
	it("clears stale pending send intent from a completed canonical snapshot that already acknowledged the prompt text", async () => {
		...
		const result = await Effect.runPromise(
			Effect.result(store.connection.sendMessage("session-1", "copilot acknowledged prompt"))
		);
		expect(Result.isSuccess(result)).toBe(true);
		expect(store.read.getSessionPendingSendIntent("session-1")).not.toBeNull();
	...
	it("applies multi-command transcript and graph patch deltas in order and clears pending send intent after both apply", async () => {
		...
		const sendResult = await Effect.runPromise(
			Effect.result(store.connection.sendMessage("session-1", "multi-command ordering test"))
		);

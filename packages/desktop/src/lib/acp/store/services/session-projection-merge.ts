 ...
 * project path -- there is no canonical fact to display.
 */
export function mergeProjectionSessions(
	...
	for (const projected of projectedSessions) {
		...
		if (existing !== undefined) {
			const projectedUpdatedAt = isoToDate(projected.updatedAt);
		...
		merged.push({
			...
			title: projected.title,
			createdAt: isoToDate(projected.createdAt),
			updatedAt: isoToDate(projected.updatedAt),

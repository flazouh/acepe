...
function createManager(deps: {
	...
	(deps.stateReader.getSessionAvailableModes as ReturnType<typeof vi.fn>).mockImplementation(
		(sessionId: string) => deps.capabilities.readCapabilities(sessionId)?.availableModes ?? null
	);
...
describe("SessionConnectionManager.connectSession", () => {
	...
	it("does not mutate historical content while attaching transport to a hydrated session", async () => {
		...
		const result = await runToResult(
			manager.connectSession(sessionId, createMockEventHandler(), {
	...
	it("reconciles once the applied lifecycle envelope reports canSend, even though readiness capabilities never populate", async () => {
		const manager = createManager({
			stateReader,
			stateWriter,
			transientProjection,
...
describe("SessionConnectionManager.createSession", () => {
	...
	// "acceptEdits"/"plan"/"bypassPermissions" (Claude/Provider.ts's
...
describe("SessionConnectionManager autonomous policy", () => {
	...
	it("does not reconnect sessions to enable Autonomous", async () => {
		...
		const result = await runToResult(
			manager.setAutonomousEnabled(sessionId, true, createMockEventHandler())
		);
...
describe("SessionConnectionManager.disconnectSession", () => {
	it("clears only local transient projection when disconnecting a session", async () => {
		...
		const stateReader: ISessionStateReader = {
			...
			getSessionTurnState: vi.fn(() => null),
			getSessionLastTerminalTurnId: vi.fn(() => null),

...
// Derived: check if sidebar should be visible (open and has projects)
// Hide sidebar when no projects exist to center the empty state
const showSidebar = $derived(
	viewState.sidebarOpen && projectManager.projectCount !== null && projectManager.projectCount > 0
);
	...
	<div class="flex flex-col h-full min-h-0 gap-0.5 bg-background rounded-xl p-0.5">
		...
		{#if !viewState.reviewFullscreenOpen}
			<div class="flex-1 flex min-h-0 gap-0.5 overflow-hidden">
				{#if showSidebar}
					<div class="shrink-0 flex flex-col h-full min-h-0">
						<AppSidebar {projectManager} state={viewState} />
					</div>
				{/if}
				<main
					class="flex-1 flex min-h-0 flex-col overflow-hidden rounded-lg {hasAnyPanel
						? ''

...
const clientOf = (input: {
	...
	gitCall: () => Effect.succeed({ op: "git.isRepo" as const, isRepo: false }),	events: () => Stream.fromArray(input.events),
});
		...
		Effect.runPromise(
			Effect.gen(function* () {
				...
				const deferred: RpcSessionSnapshot = {
					...
					checkpoints: snapshotWithUser.checkpoints,
	...
	Effect.runPromise(
		Effect.gen(function* () {
			...
			const client: RpcClient = {
				...
				writeTextFile: () => Effect.void,
				getDefaultShell: () => Effect.succeed("/bin/zsh"),
				gitCall: () => Effect.succeed({ op: "git.isRepo" as const, isRepo: false }),				events: () => Stream.fromArray([]),
			};

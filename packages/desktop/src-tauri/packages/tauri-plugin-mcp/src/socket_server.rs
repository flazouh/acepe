...
impl<R: Runtime> SocketServer<R> {
    ...
    pub fn start(&mut self) -> crate::Result<()> {
        ...
        let listener = match &self.socket_type {
            SocketType::Ipc { path } => {
                // Get the socket path for potential cleanup
                let socket_path = self.get_socket_path(path);

                // Clean up stale socket file if it exists (Unix domain sockets leave files behind)
                if !cfg!(target_os = "windows") {
                    let path = std::path::Path::new(&socket_path);
                    if path.exists() {
                        info!("[TAURI_MCP] Removing stale socket file: {}", socket_path);
                        if let Err(e) = std::fs::remove_file(path) {
                            info!("[TAURI_MCP] Warning: Failed to remove stale socket file: {}", e);
                        }
                    }
                }
                ...
                let ipc_listener = opts.create_sync()
                    .map_err(|e| {
                        ...
                        if e.kind() == std::io::ErrorKind::AddrInUse {
                            Error::Io(format!("Socket address already in use at path '{}'. The socket may be held by another process.", socket_path))
                        } else {
                            Error::Io(format!("Failed to create local socket at '{}': {}", socket_path, e))
                        }
                    })?;
    ...
    /// Get the socket path string for cleanup purposes
    fn get_socket_path(&self, path: &Option<std::path::PathBuf>) -> String {
        if let Some(p) = path {
            p.to_string_lossy().to_string()
        } else {
            let temp_dir = std::env::temp_dir();

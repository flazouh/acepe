...
pub async fn load_opencode_messages_from_disk(
    ...
    if let super::sqlite::OpenCodeStorageLayout::Sqlite { db_path } =
        super::sqlite::resolve_default_storage_layout()
    {
        tracing::info!(
            session_id = %session_id,
            db_path = %db_path.display(),
            "Loading OpenCode session from modern SQLite storage"
        );
        return super::sqlite::load_messages_sqlite(&db_path, session_id);
    }

...
pub async fn acp_switch_session_to_worktree(
    ...
    expected_acp_command_result(
        ...
        async {
            ...
            let session_registry = app.state::<SessionRegistry>();
            let _existing_client = session_registry
                .get(&session_id)
                .map_err(SerializableAcpError::from)?;
            ...
            let worktree = crate::git::worktree::create_managed_worktree_for_session(
                &project_path_string,
                None,
            )
            .map_err(|error| SerializableAcpError::InvalidState {
            ...
            if !setup_result.success {
                let _ =
                    crate::git::worktree::git_worktree_remove(worktree_path.clone(), true).await;
            ...
            .await
            {
                Ok(client) => client,
                Err(error) => {
                    let _ = crate::git::worktree::git_worktree_remove(worktree_path.clone(), true)
                        .await;
                    return Err(error);
                }
            };
            ...
            if let Some(old_client) = session_registry.store(
                ...
                let mut old =
                    lock_session_client(&old_client, "switch session to worktree: old lock")
                        .await?;
                old.stop();
            }
            session_registry

...
pub(super) fn response_outcome_allows(result: &Value) -> bool {
    matches!(
        result
            .get("outcome")
...
pub(super) fn selected_option_id(result: &Value) -> Option<&str> {
    result
        .get("outcome")
...
pub(super) fn extract_question_answer_map(result: &Value) -> serde_json::Map<String, Value> {
    result
...
pub(super) fn build_question_answer_map(
...
pub(super) fn question_answers_are_empty(answers: &[Vec<String>]) -> bool {
    answers.iter().all(Vec::is_empty)
}

pub(super) fn build_question_reply_text(
    questions: &[QuestionItem],
    answers: &[Vec<String>],
) -> String {
    let lines = questions
...
pub(super) fn question_request_binding_grace_duration() -> Duration {
    if cfg!(test) {
...
pub(super) fn question_request_binding_poll_interval() -> Duration {
    if cfg!(test) {
...
pub(super) async fn wait_for_question_request_binding(
...
pub(super) async fn take_stream_only_question_state(
    pending_questions: &Arc<Mutex<HashMap<String, PendingQuestionState>>>,
    question_id: &str,
) -> Option<PendingQuestionState> {
...
pub(super) async fn has_pending_stream_only_question(
    pending_questions: &Arc<Mutex<HashMap<String, PendingQuestionState>>>,
    session_id: &str,
) -> bool {
    pending_questions
...
pub(super) async fn should_suppress_update_while_awaiting_stream_only_question(
    pending_questions: &Arc<Mutex<HashMap<String, PendingQuestionState>>>,
    session_id: &str,
...
pub(super) async fn annotate_pending_question_request(

...
impl SessionRegistry {
    ...
    pub fn store(
        ...
        if let Some((_, old_entry)) = self.sessions.remove(&session_id) {
            tracing::warn!(session_id = %redacted_id, "Replacing existing session client");
            self.sessions.insert(session_id, entry);
            Some(old_entry.client)
        } else {
            tracing::info!(session_id = %redacted_id, "Session client stored");
            self.sessions.insert(session_id, entry);
            None
        }
    }
    ...
    pub fn remove(
        ...
        if result.is_some() {
            let redacted_id = redact_session_id(session_id);
            tracing::info!(session_id = %redacted_id, "Session client removed");
        }
        result
    }

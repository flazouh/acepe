...
use crate::db::repository::{SessionEventSequenceRepository, SessionMetadataRepository};
...
#[tokio::test]
async fn begin_activation_joins_ready_session_without_advancing_frontier() {
    let db = setup_db().await;
    seed_session_metadata(&db, "ready-session").await;
    let projection_registry = ProjectionRegistry::new();
    let supervisor = SessionSupervisor::new();
    assert!(supervisor.seed_checkpoint(
        "ready-session".to_string(),
    ...
    let result = supervisor
        .begin_activation(&db, &projection_registry, "ready-session")
        .await
        .expect("ready session should join existing activation");

    assert!(matches!(
        result,
        crate::acp::lifecycle::BeginActivationOutcome::Joined(checkpoint)
            if checkpoint.lifecycle == LifecycleState::ready()
    ));
    assert_eq!(
        SessionEventSequenceRepository::last_assigned_event_seq(&db, "ready-session")
            .await
            .expect("load ready session frontier"),
        None,
        "joining a ready session must not append an activation barrier",
    );
}

...
pub async fn sql_studio_explore_table(
    ...
    unexpected_command_result(
        ...
        async {
            ...
            let (safe_offset, safe_limit) =
                normalize_explorer_window(request.offset, request.limit);
            ...
            match connection.engine.as_str() {
                ...
                "postgres" => {
                    ...
                    let columns =
                        list_postgres_columns(&pool, &request.schema_name, &request.table_name)
                            .await?;
                    let qualified_table = build_qualified_table_name(
                        "postgres",
                        &request.schema_name,
                ...
                "mysql" => {
                    ...
                    let columns =
                        list_mysql_columns(&pool, &schema_name, &request.table_name).await?;
                    let qualified_table =

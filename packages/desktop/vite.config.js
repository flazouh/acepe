...
import { acepeDeferStylesheetHmr } from "./scripts/vite-defer-stylesheet-hmr.js";
...
export default defineConfig({
	...
	plugins: [
		...
		sveltekit(),
		tailwindcss(),
		acepeDeferStylesheetHmr({
			stylesheetFile: path.resolve(viteConfigDir, "src/app.css"),
			viteRoot: viteConfigDir,
		}),
	],
	...
	resolve: {
		...
		// Svelte's HMR registry lives inside the runtime module. Without deduping
		// `svelte`, the WebView loaded a second copy, so `$.hmr()` registered the
		// new component version in a registry the rendered tree did not use: the
		// update arrived, the accept callback ran, and the DOM stayed on the old
		// version. With one copy, a component edit repaints in about 0.3s.
		dedupe: ["@acepe/ui", "svelte"],
